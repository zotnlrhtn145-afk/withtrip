...
export function kakaoPlaceFeed(share: PlaceShare, origin: string, nativeAppLinks = false) {
  ...
  return { objectType: "feed", content: { title: schedule ? `${scheduleShareHeading(schedule)}\n${share.place.name || "공유한 장소"}` : share.place.name || "공유한 장소", description: (schedule ? `${scheduleShareContext(schedule)}\n${shareQuestionText(schedule.question)}` : placeShareSummary(share.place)).slice(0, 200), imageUrl: share.place.imageUrl || `${origin}/design/withtrip-share-logo.png`, link }, itemContent: { profileText: `${share.sender}님이 ${schedule ? "여행 일정을" : "이 장소를"} 공유합니다`, profileImageUrl: `${origin}/design/withtrip-share-logo.png` }, buttons: [{ title: "공유한 장소 보기", link }] }
}
