## 2026-09-26 Places 비용 절감 1차 (배포 검증 전)

- 사용자 목표: 기존 구글 비용 1/20. 달성 보장 아님. 월 5천 원 강제 상한/새 조회 제한 여부는 질문 대기, 정상 사용자 조회 차단은 도입하지 않음.
- 관리 대표사진/영업시간/장소 일괄 보완 및 두 cron은 ENABLE_PAID_PLACE_MAINTENANCE=true 명시 설정 전 기본 skipped. 자동 유료 보완 중지, 기존 사용자 데이터 삭제 없음.
- 사진: 요청보다 큰 저장본 재사용, 신규1600px master를 동시 크기 요청끼리 공유. 만료 참조 복구 성공 시 저장 경로 alias를 기록해 구형 앱의 반복 실패 재과금 방지. 사진8장 UI 유지.
- 검색: 신규 Text Search 응답의 상호/좌표/사진/평점을 바로 사용, 후보 전체 Details 확대조회 제거. 캐시 ID만 있고 장소정보 없을 때 기존 복구 유지. 검색 카드에는 최초 전화번호가 없을 수 있으며 상세에서 확인.
- 상세: 장소 ID별 Next Data Cache 24h (실패 미캐시), open_now 저장값 대신 기존 현지 영업시간 계산. 임시휴무 등 당일 변경은 최대24h 지연 가능. 응답캐시60초. 캐시기간 총비용95% 보장 아님.
- 계량: after()로 종료 후 기록을 보장하도록 보완, 진행 중 flush 기다림 및 새 버퍼 drain, 신형photo/media 분류 수정. 사용자ID별 귀속은 여전히 미구현.
- 검증: 실제 라우트 모의20회 검색→Text Search1/Details0, 상세20회→Details1/사진8장유지; 사진보관/복구/동시요청회귀 통과. 실제 운영 저장소 표본1200px→80/320/500px 세 요청302 및 실사진200image/jpeg, Google0회. 실제 양OS UI/총청구95% 미검증.
- 신규 DB스키마/유료 크롤링/EAS/OTA 없음. 기존 미배포 방문후보/일정사진 커밋은 이번 비용 배포에서 제외.
...
## 2026-09-23 찜 필터·BEST 업종 마커 운영 배포 완료
- f0d42d8(필터579f623 포함) / dpl_85oJLBMVtrxP9qKJeHr2NWxTtEsc READY, www.withtrip.co.kr 연결 확인. 실제 운영 BEST 방콕33곳 및 미쉐린209곳 조회→앱 필터 목록209/마커209 검증. 실제 로그인 웹/양OS 화면 전체경로 미검증.
- BEST 바 잔/레스토랑 식기+문구 마커 복원, 하단 이중 선정 배지·사진 팝업 유지. Figma425:4898. 앱062b148 iOS151/Android98 진행(앱 HANDOFF 참조).
- CLI 최초 팀 미지정 Not authorized, 기존 팀 scope 지정 재시도 성공. 사용자 데이터 추가변경·정식 앱스토어 출시·OTA 없음.
...
## 2026-09-22 저장 사진 무기한 재사용 운영 배포 완료
- a98cad3 / dpl_BBhfeMfnby3TJmfkwC1EnwcT8kav. CLI 운영배포 종료0, www.withtrip.co.kr 실제 GET 2회 동일 저장소 경로·이미지 바이트, POST photo/urls 동일 경로 확인.
- 저장 시점 만료 제거는 단위검사로 2000년 샘플 확인. 실제 저장소 검증은 기존 표본이며 시간 경과 시험과 구분. Google차단/키없는 수정코드 실제 저장소조회2회 성공. 양OS UI 직접실행 미검증, 공통서버 변경으로 기존 설치앱 적용; 새 EAS/OTA 없음.
- Vercel MCP는 scope403, 기존 인증 CLI 배포 성공. 사용자 데이터/사진 삭제/대량수집 없음.

## 2026-09-22 사진 저장본 기간 만료 제거 — 서버 배포 준비
- 사용자 명시 요청: 조회된 사진 파일을 자동 만료 없이 재사용. Google 영구 재사용 권한과 별개라는 설명 전달 후 운영 결정. 별도 지도 크롤러/선수집 추가 없음.
- photo GET 및 photo/urls POST의 30일 조건 제거. 기존 URL/버킷/권한 유지, 저장 조회를 Google 키 검사보다 먼저 수행. 앱·웹 동일 API 적용, 새 네이티브 빌드 불필요.
- 동일 photo_reference 해시+width 기준. 새 참조나 다른 해상도는 재수집 가능; 분산 서버 간 동시 최초 요청 완전 중복방지/출처별 영구ID 연계 미구현. 저장 실패는 기존 바이트 응답 폴백 유지. 무과금/무손실 보장 아님.
- 타입 및 저장/복구 회귀 통과. 수정 코드→실제 운영 저장소 동일 사진 2회 바이트 확인, Google키 없이 Google요청0/DB쓰기0. 운영 새 배포 검증은 후속 기록.
- 기존 문서/SQL의 Google사진 30일 허용 설명은 부정확한 과거 기록. 이번 변경은 약관 허용 확인이 아님. DB스키마/RLS/사용자사진 미변경.

## 2026-09-22 누락·만료 찜 사진 복구 / 운영 반영
- 사용자 전체 이용자 사진 복구·재발방지 승인. 서버3105982 / dpl_3B6283BucfpKQ439xN2njLMCcfzC READY, www.withtrip.co.kr.
- 사진 프록시400/404: 서버 저장된 ref→단일 Google ID만 인정, 캐시 후보 우선 후 photos 최소 조회. 실제 이미지 성공 뒤 saved_places는 id+gid+기존image_url 조건 CAS, 사용자가 바꾼 사진 보호. 응답사진 MIME 검사. 무한 재귀·실패사진 저장 방지, 기존30일캐시 유지.
- 운영870행 검사,784고유주소 중 실패248, 빈값6. 시험명3행 제외. 실제사진확인 후276행 복구, DB재조회 후246고유사진 모두 HTTP200/image 확인. 운영 자동복구도 일괄처리 전 실패사진에서200/image 확인. 신고사진 간판까지 육안 확인.
- 미복구2장소는 Google photos0 / ID·주소·좌표 없음. 다른 장소·임의 사진 미삽입. 개인정보·원본 찜 목록 문서복사 없음. 사진필드 이전값은 임시비공개 /tmp/*photo-repair-backup-private.json.
- 앱0f5e9ba 공유저장 사진준비 검사 추가. 웹에 공유 가져오기 UI는 없으므로 신설 안 함, 공통 읽기/프록시로 양쪽 복구. 다음 양OS빌드대기, EAS/OTA 없음.
- 웹·앱타입/사진복구·즉시저장 회귀 통과. 로그인 공유저장→양OS 실제화면 E2E 미검증.

## 2026-09-22 위디 실제 여행 데이터 기반 운영 검증
- 해당 여행 관리자 읽기 전용 조회/테스트 사용자 명시승인. 여행 문맥 선택→운영 concierge 실제 요청 HTTP200/23.6초/사진있는 검증 식당1곳 및 숙소거리 확인. 여행·대화 데이터 변경 없음, 개인 여행 원문/ID/좌표 기록 금지.
- 로그인 RLS 경로·widy-turn 저장·iOS/Android UI 전체 검증과 구분. 시뮬레이터는 로그아웃 상태. 앞으로 결과 선검증 원칙 AGENTS 반영. 새 배포/빌드 없음.

## 2026-09-22 위디 일본 추천 검증 탈락 후속 운영 수정
- 사용자 재신고: 도시/숙소 문맥 연결 뒤에도 후보 검증0건. 공개 검색 API로 모리야 본점4.6 확인, 한글 응답명과 일본어/영문 후보 비교 및 주소 선두 숫자 대조 문제 확인.
- 웹630cded(a5f3bea 후 타입 보완) 운영 dpl_6vVNE3uwm2RVAy2ZsVkLUBHNwNCD READY / www.withtrip.co.kr. 실패한 첫 빌드는 alias에 반영되지 않음.
- Google 검색 언어를 후보 현지문자에 맞춤(일본어/한글/영문), 생성 주소를 검색 문자열에 섞지 않음. 일본 지번 위치·전각·chome 정규화, 상호 설명 접두어 처리. 업종/평점/폐업/거리/동명복수지점 거부 유지. 캐시v7/v4 갱신, 로그에는 상태·개수만.
- 실제 운영 concierge 요청: 공개 산노미야 좌표, 오래된 와규 식당 질문 → HTTP200/21.2초/3곳(미소노 고베 본점4.4, 가와무라 산노미야 본점4.4, 이토그릴4.0). 실제 Gemini→Google 검증 성공. 창업연혁 등 생성된 설명의 모든 사실까지 검수한 것은 아님.
- 사용자 로그인 숙소로 전체대화/양OS 실기기 E2E는 미검증. 사용자 대화 추가 없음. 새 모바일빌드/OTA 없음. 운영 빌드 타입통과, 기존/신규 상호·주소·오지점·사진·문맥 회귀 통과. 로컬 중간타입검사는 수정 전 never 오류였으며 최종 운영빌드 타입 통과와 구분.

## 2026-09-22 위디 여행 문맥 운영 배포 완료
- 사용자 배포 승인. 소스526568f의 격리 git archive 배포(미커밋 eas.json/app.json 등 제외).
- dpl_FHFqe2nwvK6AwHbgSm4sJTyVLYKW READY, https://www.withtrip.co.kr 및 withtrip.co.kr 운영 alias 확인.
- 운영 홈200, 위디 GET/POST 비인증401 확인. 빌드 TypeScript 통과. 실제 로그인 여행의 Gemini 추천·양OS 실기기 답변 E2E는 미검증.
- iOS148/Android95 기존 요청 형식 호환 서버 변경, 새 모바일 빌드/OTA 없음. 새 질문부터 반영, 과거 답변은 자동 재작성하지 않음.

## 2026-09-22 위디 여행 문맥 복구 — 다음 배포 대기
- 사용자 승인 범위: Gemini에 여행 도시/기간, 숙소 이름·주소·체크인/아웃·좌표, 이동편 출발/도착 날짜·시각, 기존 일정 장소·시각만 전달. 예약번호·탑승자·전화번호·개인 메모 제외.
- widy-turn 사용자 RLS/참여권한 확인 뒤 숙소60·이동120·일정300행 제한 조회. 앱이 보낸 첫 숙소 대신 DB 최신값 사용. 고베/오사카 도시·날짜·체크아웃 기준 선택, 다른 여행 숙소ID/불명확한 숙소·좌표는 거부/확인 질문.
- 라우터·장소추천·블록초안에 같은 여행표 전달(최대16000자). 숙소 좌표 우선으로 도시검색 실패 방지. 생성 후보10→최대6, 시간초과/HTTP/429/빈응답 구분, 개인여행 내용 로그 없음.
- 웹 채팅 UI 신규 추가 없음. 서버 공통 수정으로 iOS148/Android95 요청 형식과 호환. 운영배포·새 모바일빌드 없음.
- 웹 타입, widy-trip-context/widy-turn/widy-generation/concierge-michelin 및 앱 city-center/september-21 회귀 통과. 합성 여행표 기반 검증이며 운영 AI 실제 답변·양OS 실기기 E2E는 미검증.
...
## 2026-09-22 BEST 전 국가 대조 / 운영 검증 대기
- 기존 공식1362기록 중1214연결/148미연결, 장소ID668개. 전 국가642미연결그룹 조회 +49주소보완 검색. 555그룹 검증,87그룹 미확인. 전체완료 아님. IDs/공식근거 docs/world-best-global-link-audit-2026-09-21.json.
- 세계/권역/식당/바 선정순위 불변. Google 캐시의 번역 건물명이 아니라 award.name을 지도·목록에 표시. API는 100ID씩 DB조회,30일유효/폐업제외, 실패503; 필터에서 유료조회 없음.
- 앱 shared 동기화. 오사카3곳7선정 회귀 통과. 미확인목록 앱 docs/월드베스트-전체연결-점검-2026-09-22.md. API 운영배포 후 응답 재검증 필요; 양OS실기기 및 새모바일빌드 미실시.

## 2026-09-21 미완료 두 신고 후속 — 추가 결정 대기
- 개인 AI: ChatGPT 구독은 API 별도 과금. 직접 구독량 연동을 단순 미구현이라고 설명한 내용을 정정. 개인 API키(별도 비용) 또는 기존10회 유지 선택을 사용자에게 요청, 응답 대기. 기존 두 계정 예외/하루10회 회귀 통과.
- BEST: 공식 프로필258개 읽기 확인(주소255), 이름 일치하는 자료와 수동대조로 주소295개 보완. 기존 캐시의10개 업장/45선정기록 추가 연결: 총1362 중279연결/1083미연결. 새10개 운영 캐시 좌표·폐업·30일유효성 모두 확인. 유료 Google 조회는0회.
- app/api/world-best ID조회100개씩 분할. 회귀: 250장소/251연도기록 보존·신선도·폐업·503 실패 통과. 기존 BEST/미쉐린 회귀 통과.
- 두 신고 fixing 상태로 실제 진행 기록. 전체완료 아님. 남은 유료조회 비용 상한 선택 대기. 공식 주소/순위는 별개이며 연도·순위는 변경하지 않음.
- 다음 빌드 대기: 웹 world-best API·카탈로그 및 앱 shared동기화. 이번 운영배포·유료 빌드 없음. API키연동 UI/기능 임의 추가 없음. 근거 웹 docs/world-best-link-audit-2026-09-21.json.
...
## 2026-09-21 월드 베스트 단일 필터 통합 — 다음 빌드 대기

- 상단 식당/바를 월드 베스트 하나로 합침. 켤 때 기본 all: 음식점·바 함께 표시. 공통 The50 SVG(기존 자산) 사용, 정렬·간격 유지.
- 필터 패널 월드 베스트 섹션: 선택 안 함 / 전체 / 음식점 / 바. 상단 활성 상태, 결과 개수, 지도·목록 동일 결과 연결. 초기화 시 업종/범위 all 복원.
- shared/saved-search.ts kind=all 추가: 업종통합, 지역·범위 유지, 동일 Google ID 중복 제거. 앱 생성물 sync. 통합/지역/범위/중복 회귀 통과.
- 앱·웹 전체 TypeScript 검사 및 shared 동기화 검사 통과.
- Figma387:4900 이미지 갱신. 신규 배포·EAS·OTA 없음. 실기기 필터 조작·큰 글씨 검증은 대기.
