...
export function PlaceAwardMarks({ awards, compact = false }: { awards: PlaceAwards; compact?: boolean }) {
  ...
  return <span style={{ display: "inline-flex", alignItems: "center", gap: compact ? 7 : 12, flexWrap: "wrap", paddingBlock: compact ? 0 : 5 }}>
    {awards.michelin ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><img src="/design/saved/michelin-face.png" alt="미쉐린 가이드 선정" width={size} height={size} style={{ width: size, height: size, objectFit: "contain", boxSizing: "border-box", padding: size * .15, backgroundColor: "#c41230", borderRadius: "50%", flexShrink: 0 }} />{!compact && awards.michelinYear ? <small>{awards.michelinYear}</small> : null}</span> : null}
