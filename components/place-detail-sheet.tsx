      ...
      */}
      ...
      <div className="relative flex h-full w-full flex-col overflow-hidden bg-white md:w-[92%] md:max-w-[440px] md:shadow-2xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto px-6 pb-6">
          ...
          <Dialog open={photoOpen} onOpenChange={setPhotoOpen}><DialogContent overlayClassName="z-[99]" className="z-[100] max-w-[calc(100%-2rem)] border-0 bg-black p-0 text-white sm:max-w-4xl"><DialogTitle className="sr-only">{name} 사진</DialogTitle>{photos[photoIndex] ? <div className="relative h-[70svh] w-full"><img src={photos[photoIndex]} alt={`${name} 사진 ${photoIndex + 1}`} className="absolute inset-0 h-full w-full object-contain" /><img key={photos[photoIndex]} src={resizePlacePhotoUrl(photos[photoIndex], PHOTO_W.full)} alt="" decoding="async" className="absolute inset-0 h-full w-full object-contain" onError={e => { e.currentTarget.style.visibility = "hidden" }} /></div> : null}<div className="flex items-center justify-between px-5 pb-4 text-white"><button type="button" disabled={photoIndex === 0} onClick={() => setPhotoIndex(i => i - 1)} className="min-h-11 disabled:opacity-30">이전</button><span>{photoIndex + 1} / {photos.length}</span><button type="button" disabled={photoIndex >= photos.length - 1} onClick={() => setPhotoIndex(i => i + 1)} className="min-h-11 disabled:opacity-30">다음</button></div></DialogContent></Dialog>
