...
export function SavedPlacesView() {
  ...
  const [sendTarget, setSendTarget] = useState<Pick<SavedPlace, "placeName" | "category" | "subCategory" | "localName" | "address" | "phoneNumber" | "imageUrl" | "rating" | "reviewCount" | "lat" | "lng"> | null>(null)
  ...
  */
  ...
  const selectCategory = (key: "all" | "wish" | "trip" | "friends") => {
    ...
    if (key === "all") { setSubFilter(""); setCountry("all"); setRegion("all"); setTripFilter("all"); setBestOnly(false); setStarredOnly(false); setOpenOnly(false); setVisitedOnly(false) }
    ...
    <div className={savedStyles.screen}>
        <div className="absolute inset-0 overflow-hidden">
          ...
          <SavedMapSheet collapseKey={collapseKey} resetKey={`${tab}:${subTab}`} title={tab === "all" ? "전체보기" : tab === "friends" ? "친구 찜" : subTab === "wish" ? "나의 찜" : "여행클립 찜"} subtitle={bestOnly?`${searchAreaName||"전체 지역"} · ${bestKind==="bars"?"베스트 바":"베스트 레스토랑"}`:searchAreaName?searchAreaName:tab === "all" ? `나의 찜 ${visiblePlaces.length} · 여행클립 ${filteredTripSpots.length} · 친구 찜 ${searchedRecs.length}` : undefined} count={bestOnly ? bestResults.length : tab === "all" ? allRows.length : tab === "friends" ? searchedRecs.length : subTab === "wish" ? visiblePlaces.length : filteredTripSpots.length}>
            ...
            {tab !== "friends" && (subTab === "wish" || tab === "all") ? <div className={savedStyles.awardFilters}>{(["restaurants","bars"] as const).map(kind=><button key={kind} className={savedStyles.awardFilter} aria-pressed={bestOnly&&bestKind===kind} onClick={()=>toggleBest(kind)}><img src={BEST_FILTER_MARKS[kind]} width={44} height={51} alt=""/><span>{kind==="bars"?"베스트 바":"베스트 식당"}</span></button>)}</div>:null}
