...
export function WorldBestBadges({ name, address, localName }: { name: string; address?: string | null; localName?: string | null }) {
  const rows = worldBestFor(name, address)
  const awards = rows.length ? rows : worldBestFor(localName || "", address)
