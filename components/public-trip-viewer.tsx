  ...
  return (
    <div className="mx-auto min-h-dvh w-full max-w-lg bg-background pb-28 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-0 lg:pb-0">
      ...
      <header className="px-6 pt-8 pb-3">
        <p className="text-sm font-semibold text-slate-600">{mode === "template" ? "공개된 여행 일정" : "공유된 일정"}{trip.city ? ` · ${trip.city}` : ""}</p>
        <h1 className="mt-2 text-[27px] leading-[36px] font-semibold tracking-tight">{trip.title}</h1>
        <p className="mt-2 text-sm text-muted-foreground">{[trip.duration, `${trip.stopCount}곳`].filter(Boolean).join(" · ")}</p>
      </header>

      {mode === "share" ? (
        <div className="animate-in fade-in slide-in-from-top-2 mx-4 mt-3 flex items-center gap-2 rounded-2xl border border-primary/50 bg-white px-3.5 py-2.5 text-xs font-medium text-foreground duration-500">
      ...
      <div className="px-6 pt-3">
        <div className="flex flex-wrap items-center gap-1.5">
          {trip.themes.map((t) => (
            <span
              key={t.key}
              className="inline-flex items-center gap-1 pr-2 text-[13px] text-muted-foreground"
            >
              {t.label}
            </span>
          ))}
          {mode === "template" && trip.forkCount > 0 ? (
            <span className="ml-auto text-xs text-muted-foreground">{trip.forkCount.toLocaleString()}명이 복제했어요</span>
          ) : null}
        </div>
      ...
      {/* Day 탭 */}
      <div className="scrollbar-none mx-6 mt-5 flex gap-1.5 overflow-x-auto border-b border-slate-200">
        {trip.days.map((d) => (
          <button
            key={d.day}
            onClick={() => setDay(d.day)}
            className={cn(
              "shrink-0 border-b-[3px] px-4 py-3 text-[15px] font-medium transition-colors",
              d.day === day
      ...
      {!wide && mapStops.length > 0 ? <div className="mx-6 mt-5 h-28 overflow-hidden rounded-2xl"><TripRouteMap stops={mapStops} selectedIndex={selectedStop} onSelect={setSelectedStop} compact /></div> : null}
      <div key={day} className="px-6 pt-6">
        <h2 className="mb-5 text-lg font-bold">{day}일차 <span className="ml-2 text-sm font-normal text-slate-600">{stops.length}곳</span></h2>
        {stops.map((s, i) => <div key={`${s.placeName}-${i}`} id={`viewer-stop-${day}-${i}`} className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 flex gap-3 duration-300" style={{ animationDelay: `${Math.min(i * 45, 225)}ms` }}>
          <div className="w-14 shrink-0 whitespace-nowrap pt-1 text-[20px] font-semibold tabular-nums">{s.visitTime ? s.visitTime.slice(0, 5) : "—"}</div>
          <div className="relative min-w-0 flex-1 border-l border-neutral-200 pb-7 pl-5">
            ...
            {s.category ? <p className="mb-1 text-sm text-slate-600">{s.category}</p> : null}
            ...
            }} className="min-h-11 w-full text-left text-[18px] leading-[26px] font-semibold underline-offset-4 hover:underline">{s.placeName} <span className="whitespace-nowrap text-sm font-normal text-slate-600">상세 보기 ›</span></button>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 mx-auto w-full max-w-lg bg-gradient-to-t from-background via-background/95 to-transparent px-4 pb-5 pt-8 lg:inset-x-auto lg:left-0 lg:right-1/2 lg:mx-0 lg:max-w-none lg:px-[max(1rem,calc((100vw/2)-40rem+1rem))]">
        ...
        {mode === "template" ? (
          <button
            ...
            disabled={forking}
            className={cn(
              "flex w-full items-center justify-center gap-2 min-h-14 rounded-full py-3.5 text-[17px] font-semibold transition-all active:scale-[0.98]",
              forking
          ...
          >
            {forking ? (
              ...
              <>
                <Sparkles className="size-4" /> 이 코스로 여행 만들기
              </>
            )}
          </button>
        ) : (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45 backdrop-blur-[2px] sm:items-center">
      <div className="animate-in fade-in slide-in-from-bottom-4 relative w-full max-w-sm rounded-t-3xl bg-card p-6 pb-8 text-center shadow-2xl duration-300 sm:rounded-3xl sm:pb-6">
        ...
        <p className="mt-3 flex items-center justify-center gap-1 text-sm text-slate-600/70">
