...
const src=ts.transpileModule(fs.readFileSync('app/api/widy-turn/route.ts','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS}}).outputText;
...
function setup(opts={}){const callbacks=[],writes=[], routeCalls=[];const user=opts.auth===false?null:{id:'me'};
 ...
 if(table==='trips')return{data:{city:'Hanoi',country_code:'VN',start_date:'2026-09-20',end_date:'2026-09-22'}};
 if(table==='widy_turns')return{data:opts.turns??[]};return{data:[]}}
 c.maybeSingle=async()=>table==='trip_messages'?{data:opts.missing?null:{id:qid,trip_id:tid,content:'hello'}}:result();c.then=(yes,no)=>Promise.resolve(result()).then(yes,no);return c;}
