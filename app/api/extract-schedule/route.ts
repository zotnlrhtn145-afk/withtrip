...
function buildPrompt(days: number, transcript: string): string {
  return (
    ...
    `여기서 **실제로 정해진 일정만** 뽑아라.\n\n` +
    `규칙:\n` +
    `- "가볼까?", "좋대", "어때?" 처럼 **아직 정해지지 않은 이야기는 제외**한다.\n` +
    `- "가자", "예약했어", "거기로 하자", "1시에 만나" 처럼 **확정된 것만** 넣는다.\n` +
    `- day 는 1부터 ${days} 사이. 며칠째인지 알 수 없으면 null.\n` +
    `- time 은 "HH:MM" 24시간제. 모르면 빈 문자열.\n` +
    `- category 는 반드시 ${CATEGORIES.join(", ")} 중 하나.\n` +
    `- quote 에는 그 판단의 **근거가 된 대화 한 줄**을 그대로 옮긴다.\n` +
    `- 같은 장소가 여러 번 나오면 한 번만.\n` +
    `- 확정된 것이 없으면 빈 배열을 돌려준다. 억지로 만들지 마라.\n\n` +
    `JSON 만 출력한다:\n` +
    `{"items": [{"day": 1, "place": "성산일출봉", "time": "09:00", "category": "관광", "quote": "둘째날 아침에 성산일출봉 가자"}]}\n\n` +
    `대화:\n${transcript}`
  )
}
